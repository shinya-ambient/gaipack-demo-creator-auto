...
全工程（リサーチ／仕様・デザイン生成／実装／レビュー／QA評価）を Claude Code で実行します。
各エージェントは独立した `claude -p` 呼び出し（＝独立コンテキスト）として動作し、
役割ごとに分離してファイルでコンテキストを受け渡します。
レビュー系エージェント（2.5 / 3.5 / 5）は「生成」とは別セッションで実行することで、
...
1. **別の独立コンテキストで起動**
   レビューは生成とは別の実行単位で起動する。会話履歴・生成時の思考過程は引き継がない。
   → 生成理由を覚えていないため「自分の判断の正当化（motivated reasoning）」が起きない。
   - skill版（推奨）: レビュー系を**別のサブエージェント（Agent ツール）**として起動する。
   - CLI版（run.sh）: レビュー系を**別の `claude -p` 呼び出し**として起動する。
...
3. **第三者ペルソナを注入**
   ...
   全レビュー起動に付与する。skill版は SKILL.md のペルソナをサブエージェントのプロンプト冒頭に、
   CLI版は `run.sh` の `REVIEWER_PERSONA` を `--append-system-prompt` で渡す。
...
└── gaipack-demo-creator/     ← ハーネス本体
    ├── run.sh                ← CLI版オーケストレーター
    └── .claude/skills/
        ├── gaipack-demo-creator/SKILL.md  ← skill版オーケストレーター（推奨）
        └── 00〜05/SKILL.md                 ← 各ステージの指示書（両版が共用）
```

## 使い方

2通りの起動方法がある。どちらも同じパイプライン・同じ出力先（`../demo/`）。

### A. Claude Code の skill として実行（推奨）
...
### B. シェルスクリプトとして実行（CLI版）
```bash
cd {プロジェクト}/gaipack-demo-creator
./run.sh 'お客様名: ○○株式会社  求めているシステム: 在庫管理ダッシュボード'
```
各ステージを `claude -p` のサブプロセスとして起動する。CI やターミナルから回したい場合に使う。
